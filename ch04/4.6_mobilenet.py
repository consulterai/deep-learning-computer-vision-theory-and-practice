...
from ch03.torch_utils import VISUALISE_MODEL, GET_FLOPS
...
class SqueezeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = F.adaptive_avg_pool2d(x, [1, 1]).view(batch, -1)
        out = self.dense(out)
    ...
    def __init__(self, model_mode="LARGE", num_classes=1000,
        ...
        if model_mode == "LARGE":
            ...
            for in_channels, out_channels, kernal_size, stride, nonlinear, se, exp_size in layers:
                ...
                self.block.append(
                    MobileBlock(in_channels, out_channels, kernal_size,
                                stride, nonlinear, se, exp_size))
            self.block = nn.Sequential(*self.block)

            out_conv1_in = _make_divisible(160 * multiplier)
            out_conv1_out = _make_divisible(960 * multiplier)
            self.out_conv1 = nn.Sequential(
                nn.Conv2d(out_conv1_in, out_conv1_out, kernel_size=1,
                          stride=1),
            ...
            self.out_conv2 = nn.Sequential(
                nn.Conv2d(out_conv2_in, out_conv2_out, kernel_size=1,
                          stride=1),
                h_swish(inplace=True),
...
# avg_pool2d(x, kernel_size=[height, width]) to F.adaptive_avg_pool2d(x,[1,1])
if __name__ == '__main__':
    temp = torch.zeros((1, 3, 112, 112))
    model = MobileNetV3(model_mode="LARGE", num_classes=1000,
                        multiplier=1.0)
    print(model(temp).shape)
    GET_FLOPS(model, [1, 3, 112, 112])
    VISUALISE_MODEL(model, torch.rand([1, 3, 112, 112]), "mobilenet")
